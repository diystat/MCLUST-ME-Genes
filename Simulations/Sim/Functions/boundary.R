...
plot.boundary = function(simrun,seed){
  ...
  contour(x,y,z,level=0,add=TRUE,drawlabels=FALSE,lwd=2)
  # boundary for erroneous obs:
  contour(x,y,zz,level=0,add=TRUE,drawlabels=FALSE,lty="dashed",lwd=2)
  # boundary for mevvv:
  contour(x,y,ze,level=0,add=TRUE,drawlabels=FALSE,lty="dotted",lwd=2)
  ...
  plot(rand.samples,pch=ifelse(rs.new1[,4]==1,2,16),main="MCLUST",cex.main=2,
    col=ifelse(rs.new1[,3]==1,col.blue,"red"),xlab="",ylab="",xlim=c(xlow,xup),ylim=c(ylow,yup),cex=1.7)
  contour(x,y,ze,level=0,add=TRUE,drawlabels=FALSE,lty="dotted",lwd=2)
  par(mfrow=c(1,1))

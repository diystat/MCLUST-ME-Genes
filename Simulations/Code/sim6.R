...
simulate.data.2 = function(seed=0) {
  ...
  cholsig[,,1] = chol(sigma1)
  cholsig[,,2] = chol(sigma2)
  cholsig[,,3] = chol(sigma3)
...
test.1 = function() {
  ...
  while(k < 11){
    ...
    es.me = EstepVVV.err(param.me,samp,err)
    
    ## Run E-step of mclust
    es = estepVVV(samp,param.mc)
  ...
  me.result = ME.VVV.err(samp,z.ini,err,"identical")
  likvec = me.result$likvec
  plot(-log(diff(likvec)),type="l",ylab="-log(error)")
  abline(h=-log(1e-5),lty="dashed",col="blue")
...
test.2 = function() {
  ...
  ## Plot error for our method:
  me.result = ME.VVV.err(samp,z.ini,err,"cluster")
  likvec = me.result$likvec
  plot(-log(diff(likvec)),type="l",ylab="-log(error)")
  abline(h=-log(1e-5),lty="dashed",col="blue")

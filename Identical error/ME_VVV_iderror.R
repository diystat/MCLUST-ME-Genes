...
ME.VVV.iderr = function(data, z, err){
  ...
  while(abs(llike[k]-llike[k-1])>0.01){
    
    print(paste("iteration =",(k-1)))
  
    thetahat = MstepVVV.iderr(z, data, err) # M-step
    
    temp = EstepVVV.iderr(thetahat, data, err) # E-step
  ...
  out = list(modelname="VVV with identical error", n=n, d=p, G=G, z=zhat,

...
EstepVVV.iderr = function(param, data, err){
  n = nrow(data)
  p = ncol(data)
  G = ncol(param[[1]])
  ...
  for (i in 1:n){
    for (k in 1:G){
      inv.sum[,,k,i] = solve(sigmahat[,,k]+err)
      L = chol(inv.sum[,,k,i])
      temp = data[i,]-muhat[,k]
      denom[i] = denom[i] + tauhat[k]*piconst*(det(sigmahat[,,k]+err))^(-1/2)*exp(-1/2*crossprod(L%*%temp))
    }
  }
  ...
  for(i in 1:n){
    for(k in 1:G){
      L = chol(inv.sum[,,k,i])
      temp = data[i,]-muhat[,k]
      num = tauhat[k]*piconst*(det(sigmahat[,,k]+err))^(-1/2)*exp(-1/2*crossprod(L%*%temp))
  ...
  loglikelihood = loglik.vvv.iderr(param, data, err)

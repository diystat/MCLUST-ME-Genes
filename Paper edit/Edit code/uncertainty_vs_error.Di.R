...
##  points(rand.samples[z==2,], col="blue", pch=2);
  ...
  plot(s, res.mcme$z[,1], xlab="magnitude of error variance", ylab="changes to membership probability to cluster 1", 
       main="Changes in membership probability to cluster 1: MCLUST --> MCLUST-ME", type="n")
  ## points(s, res.mevvv$z[,1]);
  for(i in 1:N){
    arrows(s[i], res.mevvv$z[i,1], s[i], res.mcme$z[i,1], lwd=1.3, length=0.1,
           col=ifelse(unc.mevvv[i]>unc.mcme[i],"red","dodgerblue"))
  }
